...
function buildServer() {
  const server = new McpServer({ name: 'talabat-mcp', version: '1.0.0' });

  server.tool(
    'search_menu',
    'Search Talabat menu items by name or type. Returns matching items with price, eta, and restaurant.',
  ...
  server.tool(
    'get_deals',
    'Get all active deals on Talabat.',
    {},
    async () => {
      const sb = getSupabase();
      const { data, error } = await sb
  ...
  server.tool(
    'place_order',
    'Place an order on Talabat. Returns an order confirmation with order ID.',
    {
